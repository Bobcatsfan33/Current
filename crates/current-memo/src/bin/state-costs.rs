...
use current_memo::{CostModel, Memo, Sharing};
use current_plan::bind::Catalog;
use current_state::{RedbBackend, RedbFactory, StateBackend, WriteBatch};
...
fn synthetic(entries: u64, width: usize) -> u64 {
    let dir = std::env::temp_dir().join(format!("current-state-costs-{entries}-{width}"));
...
fn running_memo(rounds: usize, rows_per_round: i64) -> Vec<Round> {
    let root = std::env::temp_dir().join("current-state-costs-memo");
...
fn main() {
    ...
    println!("    \"Regenerate: cargo run --release -p current-memo --bin state-costs > testing/evidence/c8-state-costs.json\",");
    println!(
