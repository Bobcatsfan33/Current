...
//!    from nothing to the current epoch ([`current_circuit::Circuit::catch_up`]).
...
use current_circuit::{Circuit, Epoch, NodeId, SinkId};
use current_plan::bind::Catalog;
use current_sql::{CircuitNode, CircuitPlan};
...
impl Memo {
    ...
    pub fn register_sql(&mut self, sql: &str) -> Result<Handle> {
        let plan = current_sql::compile(sql, &self.catalog)?;
    ...
    fn resolve_inputs(
        ...
        for child in current_sql::children(node) {
...
mod tests {
    ...
    use super::*;
    use current_zset::{DataType, Field, Value};
