...
mod tests {
    ...
    fn hash_of(sql: &str) -> u64 {
        let plan = current_sql::compile(sql, &catalog()).expect(sql);
    ...
    fn a_common_prefix_hashes_equal_while_the_roots_differ() {
        let a = current_sql::compile("SELECT t.n AS n FROM t WHERE t.k > 1", &catalog()).unwrap();
        let b = current_sql::compile("SELECT DISTINCT t.n AS n FROM t WHERE t.k > 1", &catalog())
            .unwrap();
