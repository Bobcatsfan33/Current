...
//! query's meaning belong to `current-plan`, errors about its text to `current-sql`, and errors about
//! the dataflow to `current-circuit`; all three are forwarded unchanged, because the memo adds no
//! semantics of its own (I-8: sharing may change counters and cost, never a result byte).

use current_circuit::CircuitError;
use current_sql::SqlError;
