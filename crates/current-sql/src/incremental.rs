...
//! `current-ops`, and the incrementalizer's job is to choose it, resolve names to positions, and
...
use current_plan::bind::{Bound, Catalog, Naming};
use current_plan::plan::{Query, Source};
use current_zset::Schema;
...
pub fn incrementalize_typed(query: &Query, catalog: &Catalog) -> Result<CircuitPlan> {
    let bound = current_plan::bind(query, catalog)?;
    plan_of(query, &bound, catalog)
}
...
fn source_node(source: &Source, catalog: &Catalog) -> Result<CircuitNode> {
    match source {
        Source::Scan { table, alias } => Ok(CircuitNode::Source {
            table: table.clone(),
            alias: alias.clone(),
            schema: current_plan::bind_source(source, catalog)?,
        }),
        Source::Join { left, right, on } => {
            ...
            let schema = Schema::new(fields).map_err(current_plan::PlanError::from)?;
...
fn index_of(schema: &Schema, name: &str) -> Result<usize> {
    schema.index_of(name).ok_or_else(|| {
        SqlError::Plan(current_plan::PlanError::UnknownColumn {
            name: name.to_owned(),
