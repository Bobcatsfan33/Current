//! The binder: one `SELECT` → one [`current_plan::Query`] (`docs/SEMANTICS.md` S-9 … S-12, S-35).
//!
...
use current_plan::bind::{bind, Bound, Catalog};
...
fn bind_aggregating(
    ...
    for item in &items.scalars {
        if !key_exprs.contains(&item.value) {
            return Err(SqlError::Plan(current_plan::PlanError::ColumnNotGrouped {
...
fn qualifier(name: &str) -> Result<&str> {
    match name.split_once('.') {
        ...
        None => Err(SqlError::Plan(current_plan::PlanError::UnqualifiedColumn(
            name.to_owned(),
        ))),
    }
}
