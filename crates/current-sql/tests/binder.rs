...
use current_plan::bind::{Catalog, Naming};
use current_plan::plan::{AggFunc, BinOp, Expr, GroupBy, Named, Query, Source};
use current_plan::PlanError;
use current_sql::{bind_sql, compile, incrementalize_typed, SqlError};
...
///
/// The oracle's side of this is `current-oracle/tests/semantics.rs`; this is the same claim through
...
fn s33_the_grand_total_answers_before_any_epoch_is_sealed() {
    ...
    let circuit = match current_sql::instantiate(&plan) {
        Ok(circuit) => circuit,
...
",
    ...
    let rules: Vec<current_sql::Rule> = plan.nodes().iter().map(|n| n.rule()).collect();
    assert_eq!(
        rules,
        vec![
            current_sql::Rule::Input,
            current_sql::Rule::Input,
            current_sql::Rule::Bilinear,
            current_sql::Rule::Linear,
            current_sql::Rule::StatefulPerGroup,
            current_sql::Rule::Linear,
            current_sql::Rule::StatefulPerRow,
        ],
...
fn naming_switches_at_the_group_by() {
    ...
    let namings: Vec<Naming> = plan
        ...
        .filter_map(|node| match node {
            current_sql::CircuitNode::Filter { naming, .. } => Some(*naming),
            _ => None,
        })
        .collect();
    assert_eq!(
