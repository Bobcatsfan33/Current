//! # current-plan — the logical plan, the binder, and the scalar expression library
//!
...
//!
//! From C1 there are two implementations of the query surface: `current-oracle` recomputes from
//! scratch, `current-circuit` maintains answers from deltas. They must agree byte for byte (I-1).
