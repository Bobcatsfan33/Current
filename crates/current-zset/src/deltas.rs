...
//! an epoch is sealed. This lives in `current-zset` because it *is* the delta representation —
//! §1's "a Z-set representing what changed between two epochs" — and every crate depends on this
//! one. Its eventual home for the *write path* is `current-log` (§5.4), which arrives in C4; until
