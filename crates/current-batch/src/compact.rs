...
use current_log::{FaultInjector, Log, Seam, SyncPolicy};
...
pub fn compact(
    ...
    if anchor <= log.retained_from() {
        return Err(BatchError::Log(current_log::LogError::NothingToCompact {
            anchor,
            retained_from: log.retained_from(),
        }));
    }
    if faults.reached(Seam::CompactBeforeSnapshot) {
        return Err(BatchError::Log(current_log::LogError::InjectedFault(
    ...
    for (table, integral) in integrals {
        ...
        checksums.insert(table.clone(), (written, current_log::record::crc32(&bytes)));
    }
    let ledger = log.dedup_ledger();
    let tokens = current_log::dedup::decode(&ledger)?.len();
    fs::write(partial.join(current_log::log::DEDUP_LEDGER), &ledger)?;
    ...
    let manifest = Manifest {
        epoch: anchor,
        tables: checksums,
        dedup_crc: current_log::record::crc32(&ledger),
    };
    {
