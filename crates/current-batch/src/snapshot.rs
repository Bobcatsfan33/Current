...
//!     u.parquet
//!     DEDUP              the acknowledged tokens (current-log's format — see why below)
...
impl Manifest {
    ...
    pub fn encode(&self) -> Vec<u8> {
        ...
        format!(
            "{body}manifest={:08x}\n",
            current_log::record::crc32(body.as_bytes())
        )
        .into_bytes()
    }
    ...
    pub fn decode(bytes: &[u8]) -> Result<Manifest> {
        ...
        if current_log::record::crc32(body.as_bytes()) != expected {
...
pub fn read_table(path: &Path, schema: &Schema) -> Result<ZSetBatch> {
    ...
    let mut entries: Vec<(current_zset::Row, i64)> = Vec::new();
    for batch in reader {
...
pub fn load(dir: &Path, catalog: &BTreeMap<String, Schema>) -> Result<BTreeMap<String, ZSetBatch>> {
    ...
    for (table, (rows, crc)) in &manifest.tables {
        ...
        if current_log::record::crc32(&bytes) != *crc {
            return Err(BatchError::CorruptSnapshot {
...
mod tests {
    ...
    use super::*;
    use current_zset::{DataType, Field, Row, Value};
    ...
    fn a_table_round_trips_through_parquet() {
        let dir = std::env::temp_dir().join(format!("current-snap-{}", std::process::id()));
    ...
    fn consolidation_drops_what_is_not_there() {
        let dir = std::env::temp_dir().join(format!("current-snap-zero-{}", std::process::id()));
        fs::create_dir_all(&dir).unwrap();
        let path = table_path(&dir, "t");
    ...
    fn a_reserved_column_name_is_refused() {
        let dir = std::env::temp_dir().join(format!("current-snap-res-{}", std::process::id()));
        fs::create_dir_all(&dir).unwrap();
